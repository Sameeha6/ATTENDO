...
function Login() {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    setError(""); 
    try {const response=await axios.post("http://127.0.0.1:8000/api/login/", {username,password}); 
      if (response.status === 200) {
        const role = response.data.role || response.data.data?.role || response.data.user?.role;
  ...
  const handleForgotPassword = async () => {
    try {
      await axios.post("http://127.0.0.1:8000/api/forgot-password/", {username: fpUsername,role: selectedRole});
    ...
    <div className="flex items-center justify-center min-h-screen bg-gray-100 p-4 sm:p-6 mt-6">
      <div className="bg-white shadow-lg rounded-xl p-6 w-full max-w-md sm:max-w-xl md:max-w-3xl flex flex-col md:flex-row">
        <div className="w-full md:w-1/2 flex flex-col justify-center items-center mb-8 md:mb-0">
          <img src="https://cdn-icons-png.flaticon.com/512/18747/18747599.png"
            alt="Logo"
            className="w-16 md:w-20"/>
          <h2 className="text-2xl md:text-3xl font-bold text-gray-800 mt-4">Welcome to AttenDo</h2>
        ...
        <div className="w-full md:w-1/2 flex justify-center md:justify-start">
          <form onSubmit={handleSubmit} className="space-y-4 w-full max-w-xs sm:max-w-none">
            ...
            <div className="mt-5">
              ...
              <p className="text-sm text-blue-500 cursor-pointer mt-2 text-center" onClick={() => setModalIsOpen(true)}>
                ...
                <div className="fixed inset-0 bg-black bg-opacity-50 backdrop-blur-sm flex justify-center items-center z-50 transition-opacity">
                  <div className="bg-white rounded-2xl shadow-2xl w-[90%] max-w-md p-6 text-center">
                    ...
                    <div className="space-y-4">
                      <input placeholder="Username" value={fpUsername} onChange={(e) => setFpUsername(e.target.value)}
                        ...
                        <div className="grid grid-cols-3 gap-3 w-full">
                          {roles.map((role) => (
                            <div key={role.value} className="flex items-center text-sm">
                              <input type="radio" id={role.value} name="role" value={role.value} checked={selectedRole === role.value}
                                onChange={() => setSelectedRole(role.value)} className="mr-2"/>
                      ...
                      <button
                        ...
                        className={`px-4 py-2 rounded-lg font-semibold ${
                          !fpUsername
                            ...
                            : "bg-cyan-800 hover:bg-cyan-700 text-white"}`}>Send Password
                      </button>
                    </div>
                  </div>
                </div>
