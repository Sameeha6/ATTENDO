...
const TutorReport = () => {
  ...
  const fetchAttendanceData = async () => {
    ...
    try {
      const res = await axios.get(`http://127.0.0.1:8000/api/tutor-report-semester/`, {params: {tutor_id: tutorId,},});
          ...
          <div className="overflow-x-auto">
            <table className="min-w-full divide-y divide-gray-200">
              <thead className="bg-gray-100">
                <tr>
                  <th scope="col" 
                    className="px-6 py-3 text-left text-xs font-medium text-gray-900 uppercase tracking-wider">Reg No</th>
                  <th scope="col"
                    className="px-6 py-3 text-left text-xs font-medium text-gray-900 uppercase tracking-wider">Name</th>
                  <th scope="col"
                    className="px-6 py-3 text-left text-xs font-medium text-gray-900 uppercase tracking-wider">Semester</th>
                  <th scope="col"
                    className="px-6 py-3 text-left text-xs font-medium text-gray-900 uppercase tracking-wider">Attendance %</th>
                </tr>
              </thead>
              <tbody className="bg-white divide-y divide-gray-200">
                {filteredAttendance.map((item, index) => (
                  <tr
                    ...
                    <td className="px-6 py-2 whitespace-nowrap text-sm text-gray-900">{item.student_id}</td>
                    <td className="px-6 py-2 whitespace-nowrap text-sm text-gray-900">{item.student_name}</td>
                    <td className="px-6 py-2 whitespace-nowrap text-sm text-gray-500">{item.semester}</td>
